                ...
                if tool then
                    ...
                    for _, npc in ipairs(npcs) do
                        if npc.hum and npc.hum.Health > 0 then
                            ...
                            if not success then
                                warn("[Hutao hub] Failed to fire shot: " .. tostring(err))
                            end
                            if AutoReloadEnabled then
                                ...
                                if not success then
                                    warn("[Hutao hub] Failed to reload: " .. tostring(err))
                                end
                            end
                        end
                    end
...
local Players = game:GetService("Players")
local player = Players.LocalPlayer
local gameWorkspace = game:GetService("Workspace") -- Äá»•i tÃªn trÃ¡nh ghi Ä‘Ã¨
...
SaveManager:SetFolder("Hutao HUB/Dead Rails")
...
Fluent:Notify({ Title = "Hutao HUB", Content = "Dead Rails script loaded successfully!", Duration = 5 })
